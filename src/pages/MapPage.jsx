    ...
    <>
      <div className="page map">
        ...
        <header className="page-head fade-in">
          ...
          <p className="kicker">
            Explore Coppell’s real organizations, mutual aid groups, and community anchors.
          </p>
        </header>

        <div className="map-grid">

          {/* STATIC MAP — NO JITTER, NO ANIMATION */}
          <div className="map-pane grid-overlay">
            <svg className="noir-map" viewBox="0 0 100 100">
              ...
              {/* CLICKABLE PINS — NOW WITH BIGGER HIT AREA */}
              {pts.map((p) => (
                <g
                  key={p.id}
                  ...
                  onClick={() => setSel(p)}
                >
                  <circle cx={p.x} cy={p.y} r={6.5} className="pin-ring-static" />
                  <circle cx={p.x} cy={p.y} r={3.2} className="pin-core-static" />
                  <text
                    x={p.x}
                    y={p.y - 9}
                    className="pin-label"
            ...
            {sel ? (
              ...
              <div className="map-card ghost fade-in-up">
                <p className="ghost-hint">
                  Select a community location to learn more.
                </p>
              </div>
            )}
          </aside>
